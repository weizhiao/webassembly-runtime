                        ...
                        bh_sockaddr_t *bh_sockaddr)
{
    ...
    case AF_INET6:
    {
        ...
        for (i = 0; i < sizeof(bh_sockaddr->addr_bufer.ipv6) / sizeof(bh_sockaddr->addr_bufer.ipv6[0]);
             i++)
        {
            uint16 part_addr = addr->sin6_addr.s6_addr[i * 2] | (addr->sin6_addr.s6_addr[i * 2 + 1] << 8);
                        ...
                        bh_sockaddr_t *src_addr)
{
    ...
    {
        if (sockaddr_to_bh_sockaddr((struct sockaddr *)&sock_addr, src_addr) == BHT_ERROR)
        {
            return -1;
        }
    }
...
{
    return (info->ai_family == AF_INET || info->ai_family == AF_INET6) && (info->ai_socktype == SOCK_DGRAM || info->ai_socktype == SOCK_STREAM) && (info->ai_protocol == IPPROTO_TCP || info->ai_protocol == IPPROTO_UDP);
}
...
int os_socket_get_send_buf_size(bh_socket_t socket, size_t *bufsiz)
{
    ...
    if (getsockopt(socket, SOL_SOCKET, SO_SNDBUF, &buf_size_int, &bufsiz_len) != 0)
    {
        return BHT_ERROR;
    }
...
int os_socket_get_recv_buf_size(bh_socket_t socket, size_t *bufsiz)
{
    ...
    int buf_size_int;
    socklen_t bufsiz_len = sizeof(buf_size_int);
    if (getsockopt(socket, SOL_SOCKET, SO_RCVBUF, &buf_size_int, &bufsiz_len) != 0)
    {
        return BHT_ERROR;
    }
    *bufsiz = (size_t)buf_size_int;
...
int os_socket_set_tcp_keep_idle(bh_socket_t socket, uint32 time_s)
{
...
int os_socket_get_tcp_keep_idle(bh_socket_t socket, uint32 *time_s)
{
    assert(time_s);
    int time_s_int;
    socklen_t time_s_len = sizeof(time_s_int);
#ifdef TCP_KEEPIDLE
    if (getsockopt(socket, IPPROTO_TCP, TCP_KEEPIDLE, &time_s_int, &time_s_len) != 0)
    {
        return BHT_ERROR;
    }
    *time_s = (uint32)time_s_int;
    return BHT_OK;
#elif defined(TCP_KEEPALIVE)
    if (getsockopt(socket, IPPROTO_TCP, TCP_KEEPALIVE, &time_s_int, &time_s_len) != 0)
    {
        return BHT_ERROR;
    }
    *time_s = (uint32)time_s_int;
...
int os_socket_set_tcp_keep_intvl(bh_socket_t socket, uint32 time_s)
{
    int time_s_int = (int)time_s;
#ifdef TCP_KEEPINTVL
...
int os_socket_get_tcp_keep_intvl(bh_socket_t socket, uint32 *time_s)
{
#ifdef TCP_KEEPINTVL
    assert(time_s);
    int time_s_int;
    socklen_t time_s_len = sizeof(time_s_int);
    if (getsockopt(socket, IPPROTO_TCP, TCP_KEEPINTVL, &time_s_int, &time_s_len) != 0)
    {
        return BHT_ERROR;
    }
    *time_s = (uint32)time_s_int;
...
{
    if (setsockopt(socket, IPPROTO_IP, IP_MULTICAST_TTL, &ttl_s, sizeof(ttl_s)) != 0)
    {
        return BHT_ERROR;
    }
...
{
    socklen_t opt_len = sizeof(ttl_s);
    if (getsockopt(socket, IPPROTO_IP, IP_MULTICAST_TTL, ttl_s, &opt_len) != 0)
    {
        return BHT_ERROR;
    }
