                              ...
                              uint32 tbl_idx)
{
    ...
    if (!(tbl_inst = LLVMBuildInBoundsGEP2(comp_ctx->builder, INT8_TYPE,
                                           func_ctx->wasm_jit_inst, &offset, 1,
                                           "tbl_inst")))
    {
...
bool wasm_jit_compile_op_elem_drop(JITCompContext *comp_ctx, JITFuncContext *func_ctx,
                                   uint32 tbl_seg_idx)
{
    LLVMTypeRef param_types[2], ret_type, func_type, func_ptr_type;
    LLVMValueRef param_values[2], ret_value, func, value;

    /* void wasm_jit_drop_table_seg(AOTModuleInstance *, uint32 ) */
    param_types[0] = INT8_PTR_TYPE;
    param_types[1] = I32_TYPE;
    ret_type = VOID_TYPE;

    if (comp_ctx->is_jit_mode)
        GET_wasm_jit_FUNCTION(llvm_jit_drop_table_seg, 2);
    else
        GET_wasm_jit_FUNCTION(wasm_jit_drop_table_seg, 2);

    param_values[0] = func_ctx->wasm_jit_inst;
...
static bool
wasm_jit_check_table_access(JITCompContext *comp_ctx, JITFuncContext *func_ctx,
                            uint32 tbl_idx, LLVMValueRef elem_idx)
{
    ...
    if (!(offset = I32_CONST(get_tbl_inst_offset(comp_ctx, func_ctx, tbl_idx) + offsetof(AOTTableInstance, cur_size))))
    {
        HANDLE_FAILURE("LLVMConstInt");
        goto fail;
    }
...
bool wasm_jit_compile_op_table_get(JITCompContext *comp_ctx, JITFuncContext *func_ctx,
                                   uint32 tbl_idx)
{
    ...
    if (!wasm_jit_check_table_access(comp_ctx, func_ctx, tbl_idx, elem_idx))
    {
        goto fail;
    }

    /* load data as i32* */
    if (!(offset = I32_CONST(get_tbl_inst_offset(comp_ctx, func_ctx, tbl_idx) + offsetof(AOTTableInstance, elems))))
    {
        HANDLE_FAILURE("LLVMConstInt");
        goto fail;
    }
...
bool wasm_jit_compile_op_table_set(JITCompContext *comp_ctx, JITFuncContext *func_ctx,
                                   uint32 tbl_idx)
{
...
bool wasm_jit_compile_op_table_init(JITCompContext *comp_ctx, JITFuncContext *func_ctx,
...

{
    ...
    if (comp_ctx->is_jit_mode)
        GET_wasm_jit_FUNCTION(llvm_jit_table_init, 6);
    else
        GET_wasm_jit_FUNCTION(wasm_jit_table_init, 6);
...
bool wasm_jit_compile_op_table_copy(JITCompContext *comp_ctx, JITFuncContext *func_ctx,
                                    uint32 src_tbl_idx, uint32 dst_tbl_idx)
{
    ...
    if (comp_ctx->is_jit_mode)
        GET_wasm_jit_FUNCTION(llvm_jit_table_copy, 6);
    else
        GET_wasm_jit_FUNCTION(wasm_jit_table_copy, 6);
...
bool wasm_jit_compile_op_table_size(JITCompContext *comp_ctx, JITFuncContext *func_ctx,
                                    uint32 tbl_idx)
{
...
bool wasm_jit_compile_op_table_grow(JITCompContext *comp_ctx, JITFuncContext *func_ctx,
                                    uint32 tbl_idx)
{
    ...
    if (comp_ctx->is_jit_mode)
        GET_wasm_jit_FUNCTION(llvm_jit_table_grow, 4);
    else
        GET_wasm_jit_FUNCTION(wasm_jit_table_grow, 4);
...
bool wasm_jit_compile_op_table_fill(JITCompContext *comp_ctx, JITFuncContext *func_ctx,
                                    uint32 tbl_idx)
{
    ...
    if (comp_ctx->is_jit_mode)
        GET_wasm_jit_FUNCTION(llvm_jit_table_fill, 5);
    else
        GET_wasm_jit_FUNCTION(wasm_jit_table_fill, 5);
