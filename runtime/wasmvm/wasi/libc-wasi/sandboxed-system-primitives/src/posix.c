    ...
    REQUIRES_EXCLUSIVE(pt->lock)
{
    if (pt->size <= min || pt->size < (pt->used + incr) * 2) {
        ...
        struct fd_prestat *prestats =
            os_malloc((uint32)(sizeof(*prestats) * size));
        if (prestats == NULL)
            return false;

        if (pt->prestats && pt->size > 0) {
            memcpy(prestats, pt->prestats, (uint32)(sizeof(*prestats) * pt->size));
        }
    ...
    REQUIRES_EXCLUSIVE(ft->lock)
{
    if (ft->size <= min || ft->size < (ft->used + incr) * 2) {
        ...
        struct fd_entry *entries =
            os_malloc((uint32)(sizeof(*entries) * size));
        if (entries == NULL)
            return false;

        if (ft->entries && ft->size > 0) {
            memcpy(entries, ft->entries,
    ...
    __wasi_fd_t fd, char *path, size_t path_len)
{
    ...
    memcpy(path, prestat->dir, (uint32)path_len);
...
readlinkat_dup(int fd, const char *path, size_t *p_len)
{
    ...
    for (;;) {
        char *newbuf = os_malloc((uint32)len);
        ...
        if (buf != NULL) {
            memcpy(newbuf, buf, (uint32)len_org);
            os_free(buf);
        }
    ...
    size_t *nevents) NO_LOCK_ANALYSIS
{
    ...
    struct fd_object **fos =
        os_malloc((uint32)(nsubscriptions * sizeof(*fos)));
    if (fos == NULL)
        return __WASI_ENOMEM;
    struct pollfd *pfds =
        os_malloc((uint32)(nsubscriptions * sizeof(*pfds)));
    if (pfds == NULL) {
        os_free(fos);
        return __WASI_ENOMEM;
    }
    ...
    __wasi_size_t *max_info_size)
{
    bh_addr_info_t *wamr_addr_info =
        os_malloc(addr_info_size * sizeof(bh_addr_info_t));
    ...
    char **argv, char *argv_buf)
{
    ...
    memcpy(argv_buf, argv_environ->argv_buf, (uint32)argv_environ->argv_buf_size);
    return __WASI_ESUCCESS;
}
    ...
    if (pt->prestats) {
        for (uint32 i = 0; i < pt->size; i++) {
            if (pt->prestats[i].dir != NULL) {
                os_free((void *)pt->prestats[i].dir);
            }
        }
...
addr_pool_insert(struct addr_pool *addr_pool, const char *addr, uint8 mask)
{
    ...
    if (!(next = os_malloc(sizeof(struct addr_pool)))) {
        return false;
    }
    ...
    if (os_socket_inet_network(true, addr, &target) != BHT_OK) {
        ...
        next->type = IPv6;
        memcpy(next->addr.ip6, target.ipv6,
                    sizeof(target.ipv6));
    }
    else {
        next->type = IPv4;
                ...
                bh_ip_addr_buffer_t *target)
{
    ...
    if (addr_pool_entry->type == IPv4) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            memcpy(&basebuf[i * sizeof(partial_addr_ip6)], &partial_addr_ip6,
